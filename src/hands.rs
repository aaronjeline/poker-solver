...
pub const ALL_HANDS: usize = 2_598_960;
...
impl Hand {
    ...
    pub fn score(self) -> Entry {
        ...
        for i in 0..5 {
            let (value, suit) = self.0[i].into_inner();
            values[i] = value.0;
            suits[i] = suit.into();
        }
...
struct Hands {
    // Current state: 5 card indices in increasing order
...
impl Hands {
    pub fn new() -> Self {
        // Start with the first combination: [0, 1, 2, 3, 4]
        Self {
            state: Some([0, 1, 2, 3, 4]),
        }
    }
}
...
impl Iterator for Hands {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        for i in (0..5).rev() {
            ...
            // Position i can be incremented if current[i] < MAX_CARD - (4 - i)
            if current[i] < MAX_CARD - (4 - i) as u8 {
                pos = Some(i);
                break;
            }
        }
